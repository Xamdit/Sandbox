...
metadata AS (
  SELECT
    ...
    END AS obj_typ,
    RIGHT(
      '000' + CAST(tut.ORDINAL_POSITION AS VARCHAR(3)),
      3
    ) AS ord_pos,
    tut.column_name AS column_nm,
    COALESCE(tut.data_type, 'unknown') + CASE
  ...
  FROM
    ...
    INNER JOIN baseTbl bt ON bt.SchemaName = tut.TABLE_CATALOG
...
descr AS (
  SELECT
    ...
    bt.table_name AS table_nm,
    tut.column_name AS column_nm,
    STRING_AGG(CAST(de.value AS VARCHAR(1024)), '.  ') WITHIN GROUP (
      ORDER BY
        de.value
    ) AS description
  FROM
...
metadata_keys AS (
  SELECT
    ...
    table_nm,
    column_nm,
    STRING_AGG(key_typ, ',') WITHIN GROUP (
      ORDER BY
        key_typ
    ) AS is_key
  FROM
    (
      ...
      FROM
        ...
        INNER JOIN INFORMATION_SCHEMA.KEY_COLUMN_USAGE kcu ON cons.TABLE_CATALOG = kcu.TABLE_CATALOG
...
FROM
  metadata md
  LEFT JOIN descr de ON de.schema_nm = md.schema_nm
  AND de.table_nm = md.table_nm
  AND de.column_nm = md.column_nm
  LEFT JOIN metadata_keys mk ON mk.schema_nm = md.schema_nm
